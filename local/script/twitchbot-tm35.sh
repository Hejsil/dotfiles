...
mon_names=$(printf "%s" "$in" | rg '\.pokemons\[\d*\]\.name=')
type_names=$(printf "%s" "$in" | rg '\.types\[\d*\]\.name=')
ability_names=$(printf "%s" "$in" | rg '\.abilities\[\d*\]\.name=')
trainer_names=$(printf "%s" "$in" | rg '\.trainers\[\d*\]\.name=')
names=$(printf "%s" "$in" | rg '\.name=')
descriptions=$(printf "%s" "$in" | rg '\.description=')
trainer_parties=$(printf "%s" "$in" | rg '\.trainers\[\d*\]\.party\[\d*\]')
...
evo_cmd() {
    method=$3
    printf "%s" "$method" | rg -i 'level|lvl' >/dev/null && evo_lvl_cmd "$1" "$2" "$4"
}
...
static_mon_cmd() {
    ...
    printf "%s" "$in" | rg "(\.static_pokemons\[\d*\]\.species=)$target" -r "\${1}$dest"
}
...
trainer_party_cmd() {
    ...
    party_size=$(printf "%s" "$party" | wc -l)
